...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_SESSION['user_id'])) {
  ...
  header("Location: " . BASE_URL . "/vehicles/reviews.php?vehicle_id=$vehicle_id");
  exit();
}
...
<main class="container review-page">
  ...
  <?php if (isset($_SESSION['user_id'])): ?>
    ...
    <?php if (!$alreadyReviewed): ?>
      <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) . '?vehicle_id=' . $vehicle_id ?>" method="POST" class="review-form">
    ...
    <?php endif; ?>
  <?php else: ?>
    <p class="notice">You must <a href="<?= BASE_URL ?>/login.php">login</a> to post a review.</p>
  <?php endif; ?>
